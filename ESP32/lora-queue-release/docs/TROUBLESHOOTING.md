...
| Board resets a few ms after `wifi_connecting`, over and over | `Brownout detector was triggered` in the serial output, and whether a `net_tx_power` record appears between the two | **No `net_tx_power` record** means it died inside `WiFi.mode()`, powering up and calibrating the RF front end, before any transmission - so no power setting can avoid it. This is the supply: fit the battery (the board is designed around it as the reservoir), use a short cable on a powered port, or add 220-470 uF across 3V3/GND. **With a `net_tx_power` record** it reached the transmit stage, and `config set wifi_tx_dbm 2` is worth trying. Either way the firmware stops after the second brownout rather than looping. |
| `net` says `HELD OFF - N brownout resets` | `version`, read `last:` | Working as designed. The uplink is the largest current draw on the board and it has knocked the supply over twice, so it stays down and the node stays alive and diagnosable. Fix the supply, then **power-cycle** - neither a `reboot` nor ten minutes of clean uptime clears it, because a node running with its uplink switched off tells you nothing about whether the uplink works. |
