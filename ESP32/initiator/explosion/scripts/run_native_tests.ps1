...
.DESCRIPTION
    ...
    thing as a green `pio test -e native` on a machine that has gcc. Use
    whichever the machine can do; CI can use either.
...
# --- sources --------------------------------------------------------------
# Everything in lib/ is the pure logic: the transition table, the frame codec
# and the replay guard. Nothing under src/ is here, because all of it needs
# Arduino.
$sources = @(
