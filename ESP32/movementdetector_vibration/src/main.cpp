 ...
 * ------------------------
 * A PIR motion sensor (HC-SR501) lights an LED and buzzes out the Imperial
 * March rhythm on a vibration motor, and an I2C OLED (SSD1306, 128x64 @ 0x3C)
 * shows "ON" while motion is active, "OFF" otherwise.
 *
 * Wiring:
 *   PIR OUT   -> GPIO 13
 *   LED (+)   -> GPIO 4  (via 220-330 Ohm resistor, cathode to GND)
 *   Motor IN  -> GPIO 5  (vibration motor MODULE; VCC to 3.3V, GND to GND)
...
constexpr uint8_t MOTOR_PIN = 5;   // Vibration motor module IN; driven by vibration.cpp
...
void loop() {
  // Keep the vibration pattern moving on every pass - it never blocks.
  vibrationUpdate();
  ...
  if (motion) {
    ...
    if (!ledOn) {
      ...
      vibrationStart();  // one full play-through per trigger
      showState(true);
    ...
    digitalWrite(LED_PIN, LOW);
    showState(false);
    // The pattern is deliberately left to finish rather than being cut off
  ...
  if (millis() - lastBeat >= 1000) {
    lastBeat = millis();
    Serial.printf("alive  motion=%d  ledOn=%d  vibrating=%d\n", motion, ledOn,
                  vibrationIsActive());
  }

  // Short poll interval so the 150 ms pulses keep their timing.
  delay(5);
}
